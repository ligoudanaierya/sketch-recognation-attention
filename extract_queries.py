...
with tf.variable_scope('siamese') as scope:
	...
	kernel = tf.get_variable(name='attention_conv3_weights',initializer=tf.truncated_normal([1,1,512,512],dtype=tf.float32,stddev=1e-1))
	conv = tf.nn.conv2d(x, kernel,[1,1,1,1],padding='SAME')
	conv = tf.nn.relu(conv)
	kernel2 = tf.get_variable(name='attention_conv4_weights', initializer=tf.truncated_normal([1,1,512,1],dtype=tf.float32, stddev=1e-1))
	conv2 = tf.nn.conv2d(conv,kernel2,[1,1,1,1], padding='SAME')
	attention_score2= tf.nn.relu(conv2)
	x = x * attention_score2
...
with tf.Session() as sess:
    ...
    saver.restore(sess,'model_512')
    for root, dir, f in os.walk('source_edge'):
        for p in f :
            path = os.path.join(root,p)
            print(path)
            '''name = path.replace('source_edge','source-features')
            name2 = path.replace('source', 'score-map')
            image = cv2.imread(path)
            print(name1)
            image = cv2.resize(image, (224,224), interpolation=cv2.INTER_CUBIC)
            #image = image.reshape((1,224,224,3))
            #score = sess.run(attention_score2,feed_dict={input:image})
            show= cv2.imread(name2,0)
            show = show * 10
            ...
            plt.axis('off')
            plt.savefig(name1)'''
            name = path.replace('source_edge','source-features').replace('.jpg','')
            image = cv2.imread(path)
            print(name)
